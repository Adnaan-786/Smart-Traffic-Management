...
# Object Counter is built on first use so importing this module stays cheap
counter = None

def get_counter():
    """Returns the shared Object Counter, creating it on first call."""
    global counter
    if counter is None:
...
def get_vehicle_condition(filePath):
    ...
    result = get_counter().count(frame)
